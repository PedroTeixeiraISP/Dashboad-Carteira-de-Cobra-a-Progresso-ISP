...
def carregar_dados(file_bytes: bytes):
    with pd.ExcelFile(BytesIO(file_bytes), engine="pyxlsb") as xls:
        ...
        # 2. Carregar Aba de Bloqueados (Busca flexível por qualquer nome com 'bloq')
        df_bloqueados_aba = pd.DataFrame()
        aba_bloq_alvo = next((a for a in abas if "bloq" in str(a).lower()), None)
...
try:
    ...
    df, df_bloqueados_aba, lista_abas = carregar_dados(file_bytes)
...
with aba3:
    if not base_bloqueados_aba.empty:
        ...
        st.warning("⚠️ Não foi encontrada nenhuma aba contendo variações de 'bloq' ou a aba está sem registros.")
        st.info(f"📋 **Abas encontradas na planilha do GitHub:** `{lista_abas}`")
